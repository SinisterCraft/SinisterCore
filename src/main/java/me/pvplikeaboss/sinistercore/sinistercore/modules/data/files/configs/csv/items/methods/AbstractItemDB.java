...
import me.pvplikeaboss.sinistercore.sinistercore.utilites.typeutils.VersionUtil;
...
public abstract class AbstractItemDB implements IItemDB {
        protected final SinisterCore plugin;
        private final Map<PluginKey, ItemResolver> resolverMap = new HashMap<>();
        protected boolean ready = false;

        public AbstractItemDB(final SinisterCore plugin) {
            this.plugin = plugin;
        }
        ...
        protected ItemStack tryResolverDeserialize(final String id) {
            for (final PluginKey key : resolverMap.keySet()) {
        ...
        @Override
        public List<ItemStack> getMatching(final PlayerObject user, final String[] args) throws Exception {
            final List<ItemStack> is = new ArrayList<>();

            if (args.length < 1) {
                is.add(user.getPlayer().getItemInHand().clone());
            } else if (args[0].equalsIgnoreCase("hand")) {
                is.add(user.getPlayer().getItemInHand().clone());
            } else if (args[0].equalsIgnoreCase("inventory") || args[0].equalsIgnoreCase("invent") || args[0].equalsIgnoreCase("all")) {
                for (final ItemStack stack : user.getPlayer().getInventory().getContents()) {
